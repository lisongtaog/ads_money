...
<head>
    ...
    <title>Ads Money | Game Trend</title>
...
<script>
    ...
    function queryData() {
        ...
        var columns = [
            {data: 'date'},
            {data: 'roundStartActiveUserCount'},
            {data: 'roundStartActiveUserTrend'},
            {data: 'roundWinActiveUserCount'},
            {data: 'roundWinActiveUserTrend'},
            {data: 'roundStartFreashUserCount'},
            {data: 'roundStartFreashUserTrend'},
            {data: 'roundWinFreashUserCount'},
            {data: 'roundWinFreashUserTrend'},
            {data: 'freashUserRoundStartCount'},
            {data: 'freashUserRoundStartTrend'},
            {data: 'freashUserRoundWinCount'},
            {data: 'freashUserRoundWinTrend'},
            {data: 'avgSumFreasherRoundStart'},
            {data: 'avgSumFreasherRoundWin'},
        ];
        ...
        $('#metricTable').DataTable({
            ...
            "ajax": function (data, callback, settings) {
                ...
                $.post("game_trend_old/query", postData, function (data) {
                    if (data && data.ret == 1) {
                        var list = [];
        ...
        $.post("game_trend_old/get", {
            ...
            if (data && data.ret == 1) {
                ...
                if (list.length > 0) {
                    ...
                    var labels = [];
                    var roundStartActiveUserCount = [];
                    var roundWinActiveUserCount = [];
                    var roundStartFreashUserCount = [];
                    var roundWinFreashUserCount = [];
                    var freashUserRoundStartCount = [];
                    var freashUserRoundWinCount = [];
                    for (var i = list.length - 1; i >= 0; i--) {
                        var one = list[i];
                        var date = new Date(one.date).toLocaleDateString();
                        labels.push(date);
                        roundStartActiveUserCount.push(one.roundStartActiveUserCount);
                        roundWinActiveUserCount.push(one.roundWinActiveUserCount);
                        roundStartFreashUserCount.push(one.roundStartFreashUserCount);
                        roundWinFreashUserCount.push(one.roundWinFreashUserCount);
                        freashUserRoundStartCount.push(one.freashUserRoundStartCount);
                        freashUserRoundWinCount.push(one.freashUserRoundWinCount);
