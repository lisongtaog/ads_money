...
<head>
    ...
    <title>Ads Money | Daily Active User</title>
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                Daily Active User(AntivirusV2 Only)
            </h1>
            <ol class="breadcrumb">
                <li><a href="index.jsp"><i class="fa fa-dashboard"></i> Home</a></li>
                <li class="active">Daily Active User(AntivirusV2 Only)</li>
            </ol>
        </section>
    ...
    function queryData() {
        ...
        $('#metricTable').DataTable({
            ...
            "ajax": function (data, callback, settings) {
                ...
                $.post("daily_active_user/query", postData, function (data) {
                    if (data && data.ret == 1) {
                        var list = [];
        ...
        $.post("/daily_active_user/get", {
            ...
            if (data && data.ret == 1) {
                ...
                if (list.length > 0) {
                    var first = list[0][0];
                    $('#revenueChartTitle').text(new Date(first.date).toLocaleDateString()  + " User Distribution");

                    var labels = [];
                    var colors = ["#00c0ef", "#dd4b39", "#00a65a", "#f39c12", "#0073b7", "#e842f4", "#0aff0a"];
                    var chartConfig = {
                        'type': 'line',
                        'data' : {
                            'labels': labels,
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        for (var j = 0; j < list[i].length; j++) {
                            ...
                            avgActiveCount.push(one.avg_active_count);
                        }
                        if (list[i].length > 0) {
                            chartConfig.data.datasets.push({
                                label               : new Date(list[i][0].date).toLocaleDateString(),
